...
// TestD4ShapeDischargedArray runs ch467 — a SHAPE-DISCHARGED foreign array (D4). safeNpMatVec
// requires typed proofs that a flat FList's length matches its declared (m,k)/k shape, so a
// malformed numpy call is a COMPILE error (the elaborate sweep gates that, and a wrong-shape
// witness is rejected at refl). Here the WELL-shaped call runs real numpy through the embed:
// [[1,2],[3,4]]·[5,6] = [17,39], summed in-language to 56. The first rung of the general
// Array-dt-sh handle: shapes discharged by dependent types across the FFI boundary.
func TestD4ShapeDischargedArray(t *testing.T) {
	if _, err := exec.LookPath("cc"); err != nil {
		t.Skip("cc not in PATH")
	}
	pyCfg, err := exec.LookPath("python3-config")
	...
	s := loadListing(t, "ch467_shape_discharged_array.rune")
	p, err := s.EmitProgram("main")
	if err != nil {
		t.Fatal(err)
	}
	...
	if got := strings.TrimSpace(string(out)); !strings.HasPrefix(got, "56") {
		t.Errorf("shape-discharged matvec gave %q, want it to start 56", got)
	}
}
...
// TestD4ZeroCopyHandle runs ch475 — the ZERO-COPY ARRAY HANDLE (D4 rung 4) on the C embed.
// A 1000-element array is marshalled into numpy ONCE via npAlloc (the sole O(n) step), then
// npSumH/npScaleH operate on the opaque handle (a numpy-side key) without re-walking the
// FList: sum 2000, scale by 3 (the new array never leaves numpy), sum again 6000. The large
// array is the consumer that makes the zero-copy handle needful; observable "2000\n6000".
func TestD4ZeroCopyHandle(t *testing.T) {
	if _, err := exec.LookPath("cc"); err != nil {
		t.Skip("cc not in PATH")
	}
	pyCfg, err := exec.LookPath("python3-config")
	...
	s := loadListing(t, "ch475_zerocopy_handle.rune")
	p, err := s.EmitProgram("main")
	if err != nil {
		t.Fatal(err)
	}
	...
	if got := strings.TrimSpace(string(out)); !strings.HasPrefix(got, "2000\n6000") {
		t.Errorf("zero-copy handle gave %q, want it to start \"2000\\n6000\"", got)
	}
}
