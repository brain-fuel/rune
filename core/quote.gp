...
//
// Authored in Go+ (v3.383.0): the Val/Neutral case analyses are matches, so an
...
func (m *Machine) Quote(lvl int, v Val) Tm {
	...
	case VProp:
		return Prop
	case VEq(ty, l, r):
		return Eq(m.Quote(lvl, ty), m.Quote(lvl, l), m.Quote(lvl, r))
	case VRefl(pv):
		return Refl(m.Quote(lvl, pv))
	case VPi(name, icit, qty, _, dom, cod):
		domT := m.Quote(lvl, dom)
		body := m.Quote(lvl+1, cod(VVar(lvl)))
		return Pi(icit, qty, domT, Scope{Name: name, Body: body})
	case VLam(name, icit, qty, fbody):
		body := m.Quote(lvl+1, fbody(VVar(lvl)))
		return Lam(icit, qty, Scope{Name: name, Body: body})
	case VSig(name, qty, dom, cod):
		domT := m.Quote(lvl, dom)
		body := m.Quote(lvl+1, cod(VVar(lvl)))
		return Sig(qty, domT, Scope{Name: name, Body: body})
	case VPair(name, dom, cod, a, b):
		body := m.Quote(lvl+1, cod(VVar(lvl)))
		return Pair(m.Quote(lvl, dom), Scope{Name: name, Body: body},
			m.Quote(lvl, a), m.Quote(lvl, b))
	case VNeu(spine, _):
...
func (m *Machine) QuoteUnfold(lvl int, v Val) Tm {
	...
	if lit, ok := m.forceToNatLit(v); ok {
		return NatLit(lit.N, lit.Zero, lit.Succ)
	}
	v = m.Force(v)
	match v {
	case VU(ulvl):
	...
	case VProp:
		return Prop
	case VEq(ty, l, r):
		return Eq(m.QuoteUnfold(lvl, ty), m.QuoteUnfold(lvl, l), m.QuoteUnfold(lvl, r))
	case VRefl(pv):
		return Refl(m.QuoteUnfold(lvl, pv))
	case VPi(name, icit, qty, _, dom, cod):
	...
	case VPair(name, dom, cod, a, b):
		body := m.QuoteUnfold(lvl+1, cod(VVar(lvl)))
		return Pair(m.QuoteUnfold(lvl, dom), Scope{Name: name, Body: body},
			m.QuoteUnfold(lvl, a), m.QuoteUnfold(lvl, b))
	case VNeu(spine, _):
...
func (m *Machine) quoteSpine(lvl int, n Neutral, quoteVal func(int, Val) Tm) Tm {
	match n {
	case NVar(nlvl):
		return Var(lvl - 1 - nlvl)
	case NRef(h):
		return Ref(h)
	case NMeta(id):
		return Meta(id)
	case NCast(a, b, p, x):
		return Cast(quoteVal(lvl, a), quoteVal(lvl, b), quoteVal(lvl, p), quoteVal(lvl, x))
	case NSubst(a, x, y, prf, p, px):
		return Subst(quoteVal(lvl, a), quoteVal(lvl, x), quoteVal(lvl, y),
			quoteVal(lvl, prf), quoteVal(lvl, p), quoteVal(lvl, px))
	case NApp(fn, arg, icit):
		return App(m.quoteSpine(lvl, fn, quoteVal), quoteVal(lvl, arg), icit)
	case NFst(p):
		return Fst(m.quoteSpine(lvl, p, quoteVal))
	case NSnd(p):
		return Snd(m.quoteSpine(lvl, p, quoteVal))
	case NNatLit(nv, zero, succ):
