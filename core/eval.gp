...
func (m *Machine) Eval(env Env, t Tm) Val {
	...
	case Univ(lvl):
		return VU(lvl)
	case Pi(icit, qty, domT, cod):
		dom := m.Eval(env, domT)
		return VPi(cod.Name, icit, qty, !usesVar(cod.Body, 0), dom,
			func(v Val) Val {
				return m.Eval(env.Extend(v), cod.Body)
			})
	case Lam(icit, qty, body):
		return VLam(body.Name, icit, qty, func(v Val) Val {
			return m.Eval(env.Extend(v), body.Body)
		})
	case App(fn, arg, icit):
		return m.apply(m.Eval(env, fn), m.Eval(env, arg), icit)
	case Meta(id):
		return m.metaVal(id)
	case Prop:
		return VProp
	case Eq(ty, l, r):
		return m.EvalEq(m.Eval(env, ty), m.Eval(env, l), m.Eval(env, r))
	case Refl(tm):
		return VRefl(m.Eval(env, tm))
	case Cast(a, b, p, x):
		return m.EvalCast(m.Eval(env, a), m.Eval(env, b), m.Eval(env, p), m.Eval(env, x))
	case Subst(a, x, y, prf, p, px):
		return m.EvalSubst(m.Eval(env, a), m.Eval(env, x), m.Eval(env, y),
			m.Eval(env, prf), m.Eval(env, p), m.Eval(env, px))
	case Let(_, val, body):
		// The bound value is in-band (part of the term); the binder is
		// definitionally transparent, so the body sees the VALUE.
		return m.Eval(env.Extend(m.Eval(env, val)), body.Body)
	case Ann(term, _):
		return m.Eval(env, term)
	case Sig(qty, domT, cod):
		return VSig(cod.Name, qty, m.Eval(env, domT),
			func(v Val) Val { return m.Eval(env.Extend(v), cod.Body) })
	case Pair(domT, cod, a, b):
		return VPair(cod.Name, m.Eval(env, domT),
			func(v Val) Val { return m.Eval(env.Extend(v), cod.Body) },
			m.Eval(env, a), m.Eval(env, b))
	case Fst(p):
		return m.proj1(m.Eval(env, p))
	case Snd(p):
		return m.proj2(m.Eval(env, p))
	case NatLit(n, zero, succ):
		return m.natLitVal(n, zero, succ)
	}
}
...
func (m *Machine) proj1(v Val) Val {
	match v {
	case VPair(_, _, _, a, _):
		return a
	case VNeu(spine, unfold):
		out := VNeu{Spine: NFst{P: spine}}
		if unfold != nil {
			var memo Val
			out.Unfold = func() Val {
				if memo == nil {
...
func (m *Machine) proj2(v Val) Val {
	match v {
	case VPair(_, _, _, _, b):
		return b
	case VNeu(spine, unfold):
		out := VNeu{Spine: NSnd{P: spine}}
		if unfold != nil {
			var memo Val
			out.Unfold = func() Val {
				if memo == nil {
...
func (m *Machine) apply(fn, arg Val, icit Icit) Val {
	match fn {
	case VLam(_, _, _, body):
		return body(arg)
	case VNeu(spine, unfold):
		out := VNeu{Spine: NApp{Fn: spine, Arg: arg, Icit: icit}}
		if unfold != nil {
			var memo Val
			out.Unfold = func() Val {
				if memo == nil {
					memo = m.apply(unfold(), arg, icit)
				}
				return memo
			}
		}
