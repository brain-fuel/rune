  ...
  **M5 (matured core) is COMPLETE: C1–C6 all landed.** **TRACK A (cubical inner) is
  ...
  - **STATE SINCE v3.24 (v3.25.0 → v3.290.0, ~200 listings).** This phase-map detail
    ...
    - **D4 ML interop** — the NumPy suite (npDot/npMean/npMatSum/npVar/npMax/npNorm,
      ...
      + a stdlib import; pyFactorial → math.factorial marshalled as a DECIMAL STRING, so
      ...
      The embed now spans nat/float/bignum/1-D-list/2-D-shaped both ways + stdlib + numpy. The
      WHOLE embed runs on BOTH native backends — C and LLVM: scalars (pow/sqrt/factorial,
      v3.328.31) AND numpy (pyNpSum/Scale/MatVec, v3.328.32), baked into LL{}.EmitRuntimeFor with
      the rt_-prefixed runtime helpers (TestD4CPythonEmbedLL/TestD4CPythonNumpyLL). Full
      cross-backend native parity. Remaining: only the general `Array dt sh` ndarray handle
      (arbitrary shapes/dtypes).
    - **E3/E4 distributed** — the big one. General all-P adequacy: SOUNDNESS ch409 +
      ...
      parHaltNeutral GENERALIZED (ch464, v3.328.33): parQuietNeutral — ANY quiescent non-crash
      peer is observationally neutral (okStep p=none + isCrash p=false ⟹ visibleRun k (par p x)
      = visibleRun k x), via a double-subst okStepParQuiet, broadening the adequate class beyond
      halt-only peers. Supervision proven NECESSARY (ch461, v3.328.24): an unmonitored crash par crash halt is
    ...
    - **2026-06-21 OVERNIGHT SESSION (v3.321–v3.326.x) — env provisioning + a frontier sweep.**
      ...
      • **D4 shape-checked matrix×MATRIX** (ch447, v3.328.4) — completes the shape-safe LA trio
        ...
        RUNS on native C + LLVM too (TestD4ShapeMatMulNative, v3.328.15) — shape-safe LA across
        source + native.
