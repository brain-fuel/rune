...
Value rt_nat_add(Value a, Value b) { return rt_mkint(rt_as_int(a) + rt_as_int(b)); }
Value rt_nat_mul(Value a, Value b) { return rt_mkint(rt_as_int(a) * rt_as_int(b)); }
Value rt_nat_monus(Value a, Value b) {
  long x = rt_as_int(a), y = rt_as_int(b);
  return rt_mkint(x > y ? x - y : 0);
}
