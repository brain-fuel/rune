...
func (Go) Emit(p Program) (TargetSource, error) {
	...
	// math + strconv are ALWAYS imported (not gated to float IO): __fmtf (ECMAScript
	// Number::toString) is now unconditionally emitted so _show's float branch can
	// reach it too (Track B Task 5), mirroring the native C/LL fix.
	impSet := map[string]bool{"fmt": true, "math/big": true, "strings": true, "math": true, "strconv": true}
	...
	// gated to its only consumers (parseFloat/getFloat validation).
	// __fmtf (the canonical format via strconv 'e' shortest form + re-dress) is ALWAYS
