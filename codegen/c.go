...
func (C) Emit(p Program) (TargetSource, error) {
	...
	// Index every non-nat constructor for satCtorDispatch (the C dual of
	// wasm.go:282's index): a SATURATED ctor application lowers to a direct mkcon
	// instead of currying through the ctorfn blocks. The builtin-nat data group is
	// excluded exactly as in WASM (its zero/succ are the native bignum reps).
...
func (em *cEmitter) emitNatC(b *strings.Builder, n NatSpec) {
	...
	//     them here. (A SATURATED spine reaches this fold via satElimDispatch, which
	//     releases x after the call -- the WASM protocol; a partial application curries
	//     through b0..b2 and x's release stays deferred, a constant per-fold residue.)
...
// satCtorDispatch recognizes a SATURATED constructor application -- an AppClosure
// spine headed by a CGlobal naming a (non-nat) constructor with EXACTLY Arity args --
// and emits the K_CON DIRECTLY: mkcon(tag, name, arity) + one con_set per arg (each
// arg evaluated once into a stable local, then MOVED into its field slot). The C port
// of wasm.go satCtorDispatch (:937). Perceus keeps a recognized ctor spine's backbone
// BARE (no releases on the Clo chain), on the assumption that the emitter builds the
// K_CON directly; without this dispatch the generic apply path allocated arity-1
// intermediate curried K_CLOs per runtime ctor application that NOTHING released --
// a per-application leak that scaled in any loop building fresh structures (the
// TestCARCPressure / TestCARCRecursiveRelease shapes). A PARTIALLY applied ctor
// falls through to the curried ctorfn path unchanged (its partial-application K_CLO
// is a legitimate value).
func (em *cEmitter) satCtorDispatch(app AppClosure, locals []string) (string, bool) {
	var args []CIr
	t := CIr(app)
	for {
		ap, ok := t.(AppClosure)
	...
	c, isCtor := em.ctorByName[g.Name]
	if !isCtor || len(args) != c.Arity {
		return "", false
	}
	// Evaluate each arg into a stable local FIRST (a nested allocating arg must not
	// interleave with the con_set fills), then build + fill. Field order and the
	// tag/name/arity triple match ctorfn_<name>_<k> exactly, so the printed K_CON is
	// byte-identical to the curried path. Args arrive OWNED (annotateBareSpine
	// consumeOwning'd each leaf) and are MOVED into the field slots.
	var b strings.Builder
	b.WriteString("({ ")
	vals := make([]string, len(args))
	for i, a := range args {
		v := em.fresh()
		fmt.Fprintf(&b, "Value %s = %s; ", v, em.exprIn(a, locals))
		vals[i] = v
	}
	o := em.fresh()
	fmt.Fprintf(&b, "Value %s = mkcon(%d, %s, %d); ", o, c.Tag, cstr(c.Name), c.Arity)
	for i, v := range vals {
		fmt.Fprintf(&b, "con_set(%s, %d, %s); ", o, i, v)
	}
	fmt.Fprintf(&b, "%s; })", o)
	return b.String(), true
}

// satElimDispatch recognizes a SATURATED builtin-nat eliminator application whose
// step USES its IH (natDispatch already handled the IH-ignoring one-peel) and runs
// the b3 fold DIRECTLY: build ONLY the b3 closure (env {unit, c0, c1}) and apply it
// to x, instead of currying through the cached eliminator thunk's b0->b1->b2 chain.
// The C port of wasm.go satElimDispatch (:998). Perceus keeps a recognized nat-elim
// spine BARE, so the generic path's three intermediate partial-application K_CLOs
// were never released -- a per-evaluation leak that scaled when the fold sat inside
// an outer loop (each addN call leaked its spine). The erased motive is evaluated
// (for its consumeOwning side effects) and released -- the fold never reads env[0],
// which gets UNIT (uncounted). c0/c1 are MOVED into the b3 env (freed when the b3
// closure is released after the call; b3 retains its acc so the base survives). x is
// borrow-read by the fold and released HERE afterward -- closing the deferred
// x-release the b3 comment carried since Task 2. All four spine args arrive OWNED.
func (em *cEmitter) satElimDispatch(app AppClosure, locals []string) (string, bool) {
	if em.natElim == "" {
		return "", false
	}
	args, ok := NatElimSpine(em.natElim, app)
	...
	mot, c0, c1, x := em.fresh(), em.fresh(), em.fresh(), em.fresh()
	cl, r := em.fresh(), em.fresh()
	out := fmt.Sprintf("({ Value %s = %s; rt_release(%s); Value %s = %s; Value %s = %s; Value %s = %s; "+
		"Value %s = mkclo(&%s_b3, 3); clo_set(%s, 0, UNIT); clo_set(%s, 1, %s); clo_set(%s, 2, %s); "+
		"Value %s = apply(%s, %s); rt_release(%s); rt_release(%s); %s; })",
		mot, em.exprIn(args[0], locals), mot,
		c0, em.exprIn(args[1], locals),
		c1, em.exprIn(args[2], locals),
		x, em.exprIn(args[3], locals),
		cl, cName(em.natElim), cl, cl, c0, cl, c1,
		r, cl, x, cl, x, r)
	return out, true
}
...
func (em *cEmitter) exprIn(t CIr, locals []string) string {
	...
	case CForeign:
		...
		// loop — the byteLen-per-line leak). Memoize per call site with a block-scope
		// static, the dual of WASM's emitCachedThunk ($def_<op>) and of emitDefThunk's
		// CGlobal cache: the cache holds one never-released root for the process life.
		n := cName(x.Name)
		return fmt.Sprintf("({ static Value _fc_%s; if (!_fc_%s) _fc_%s = %s(); _fc_%s; })", n, n, n, n, n)
	case CUnit:
		return "UNIT"
	case CLit:
		switch x.Kind {
	...
	case AppClosure:
		...
		if out, ok := em.satCtorDispatch(x, locals); ok {
			return out
		}
		if out, ok := em.satElimDispatch(x, locals); ok {
			return out
		}
...
func emitStreamPrimsC(b *strings.Builder, p Program) {
	...
	// ARC (terminal pure ops): apply CONSUMES its argument, so a terminal body OWNS its
	// direct arg and must RELEASE it when it neither stores nor returns it — exactly the
	// WASM twins (byteLen_c1 wasm.go:1262 "the owned $arg is consumed (released)";
	// splitOn/jsonStrField/sqlQuote; fromNat/fmul_c2 in wasm_float.go). env[i] slots stay
	// BORROWED (freed when the curry closure dies) — only the DIRECT arg is owned.
	if usesForeign(p, "byteLen") {
		b.WriteString("static Value byteLen_c1(Value c, Value* env) { (void)env; size_t len; unsigned char* buf = d6_s2h(c, &len); free(buf); rt_release(c); return big_from_long((long)len); }\n")
...
static Value binLen_c1(Value bb, Value* env) { (void)env; Value r = big_from_long(bytes_len(bb)); rt_release(bb); return r; }
static Value binLen(void) { return mkclo(&binLen_c1, 0); }
static Value binAt_c2(Value i, Value* env) { Value bb = env[0]; long k = (big_nlimbs(i) == 0 ? 0 : big_limb(i, 0)); rt_release(i); int n = bytes_len(bb); if (k >= 0 && k < n) return big_from_long(bytes_at(bb, (int)k)); return big_from_long(0); }
...
func emitFloatPrimsC(b *strings.Builder, p Program) {
	...
	if usesForeign(p, "floatToNat") {
		b.WriteString("static Value floatToNat_c(Value x, Value* env) { (void)env; Value r = big_from_long((long)float_val(x)); rt_release(x); return r; }\n")
	...
	if usesForeign(p, "fleqN") {
		b.WriteString("static Value fleqN_c2(Value y, Value* env) { Value r = float_val(env[0]) <= float_val(y) ? big_from_long(1) : big_from_long(0); rt_release(y); return r; }\n")
	...
	if usesForeign(p, "pyNpScale") {
		b.WriteString("static Value pyNpScale_c2(Value kk, Value* env) { Value xs = env[0]; long k = IS_INT(kk) ? INT_VAL(kk) : (long)big_to_double(kk); int n = fl_len(xs); double* X = (double*)malloc(sizeof(double) * (n > 0 ? n : 1)); fl_fill(xs, X); rune_py_ensure(); PyObject* lst = PyList_New(n); for (int i = 0; i < n; i++) PyList_SetItem(lst, i, PyFloat_FromDouble(X[i])); free(X); PyObject* m = PyImport_AddModule(\"__main__\"); PyObject* g = PyModule_GetDict(m); PyObject* np = PyImport_ImportModule(\"numpy\"); Value res = mkcon(0, \"fnil\", 0); if (np) { PyDict_SetItemString(g, \"np\", np); PyDict_SetItemString(g, \"xs\", lst); char buf[80]; snprintf(buf, sizeof buf, \"(np.array(xs) * %ld).tolist()\", k); PyObject* r = PyRun_String(buf, Py_eval_input, g, g); if (r) { int rn = (int)PyList_Size(r); Value acc = mkcon(0, \"fnil\", 0); for (int i = rn - 1; i >= 0; i--) { double d = PyFloat_AsDouble(PyList_GetItem(r, i)); Value c = mkcon(1, \"fcons\", 2); con_set(c, 0, mkfloat(d)); con_set(c, 1, acc); acc = c; } rt_release(res); res = acc; Py_XDECREF(r); } else { PyErr_Clear(); } Py_XDECREF(np); } else { PyErr_Clear(); } Py_XDECREF(lst); rt_release(kk); return res; }\n")
	...
	if usesForeign(p, "pyNpMatVec") {
		b.WriteString("static Value pyNpMatVec_c4(Value v, Value* env) { Value A = env[0]; long m = IS_INT(env[1]) ? INT_VAL(env[1]) : (long)big_to_double(env[1]); long k = IS_INT(env[2]) ? INT_VAL(env[2]) : (long)big_to_double(env[2]); int an = fl_len(A), vn = fl_len(v); double* AA = (double*)malloc(sizeof(double) * (an > 0 ? an : 1)); fl_fill(A, AA); double* VV = (double*)malloc(sizeof(double) * (vn > 0 ? vn : 1)); fl_fill(v, VV); rune_py_ensure(); PyObject* pa = PyList_New(an); for (int i = 0; i < an; i++) PyList_SetItem(pa, i, PyFloat_FromDouble(AA[i])); PyObject* pv = PyList_New(vn); for (int i = 0; i < vn; i++) PyList_SetItem(pv, i, PyFloat_FromDouble(VV[i])); free(AA); free(VV); PyObject* mm = PyImport_AddModule(\"__main__\"); PyObject* g = PyModule_GetDict(mm); PyObject* np = PyImport_ImportModule(\"numpy\"); Value res = mkcon(0, \"fnil\", 0); if (np) { PyDict_SetItemString(g, \"np\", np); PyDict_SetItemString(g, \"A\", pa); PyDict_SetItemString(g, \"v\", pv); char buf[120]; snprintf(buf, sizeof buf, \"(np.array(A).reshape(%ld,%ld) @ np.array(v)).tolist()\", m, k); PyObject* r = PyRun_String(buf, Py_eval_input, g, g); if (r) { int rn = (int)PyList_Size(r); Value acc = mkcon(0, \"fnil\", 0); for (int i = rn - 1; i >= 0; i--) { double d = PyFloat_AsDouble(PyList_GetItem(r, i)); Value c = mkcon(1, \"fcons\", 2); con_set(c, 0, mkfloat(d)); con_set(c, 1, acc); acc = c; } rt_release(res); res = acc; Py_XDECREF(r); } else { PyErr_Clear(); } Py_XDECREF(np); } else { PyErr_Clear(); } Py_XDECREF(pa); Py_XDECREF(pv); rt_release(v); return res; }\n")
