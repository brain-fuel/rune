...
func emitStreamPrimsLL(b *strings.Builder, p Program) {
	...
	if usesForeign(p, "foldDir") {
		...
		b.WriteString("static Value foldDir_c5(Value s0, Value* env) { Value f = rt_mkclo(&foldDir_c6, 4); rt_clo_set(f, 0, env[0]); rt_clo_set(f, 1, s0); rt_clo_set(f, 2, env[1]); rt_clo_set(f, 3, env[2]); return f; }\n")
		b.WriteString("static Value foldDir_c4(Value step, Value* env) { Value f = rt_mkclo(&foldDir_c5, 3); rt_clo_set(f, 0, step); rt_clo_set(f, 1, env[0]); rt_clo_set(f, 2, env[1]); return f; }\n")
		b.WriteString("static Value foldDir_c3(Value suf, Value* env) { Value f = rt_mkclo(&foldDir_c4, 2); rt_clo_set(f, 0, env[0]); rt_clo_set(f, 1, suf); return f; }\n")
	...
	if usesForeign(p, "writeChunk") {
		...
		b.WriteString("static Value writeChunk_c2(Value c, Value* env) { Value f = rt_mkclo(&writeChunk_c3, 2); rt_clo_set(f, 0, env[0]); rt_clo_set(f, 1, c); return f; }\n")
	...
	if usesForeign(p, "sortFile") {
		...
		b.WriteString("static Value sortFile_c2(Value outp, Value* env) { Value f = rt_mkclo(&sortFile_c3, 2); rt_clo_set(f, 0, env[0]); rt_clo_set(f, 1, outp); return f; }\n")
	...
	if usesForeign(p, "dbApply") {
		...
		b.WriteString("static Value dbApply_c2(Value sql, Value* env) { Value f = rt_mkclo(&dbApply_c3, 2); rt_clo_set(f, 0, env[0]); rt_clo_set(f, 1, sql); return f; }\n")
...
func emitFloatIOPrimsLL(b *strings.Builder, p Program) {
	...
	if usesForeign(p, "parseFloat") {
		b.WriteString("static Value parseFloat_c(Value code, Value* env) { (void)env; size_t len; unsigned char* buf = d6_s2h(code, &len); if (!d6_validate_float(buf, len)) { free(buf); Value none = rt_mkcon(0, \"none\", 1); rt_con_set(none, 0, UNIT); return none; } char* tmp = (char*)malloc(len + 1); memcpy(tmp, buf, len); tmp[len] = 0; double d = strtod(tmp, NULL); free(tmp); free(buf); Value some = rt_mkcon(1, \"some\", 2); rt_con_set(some, 0, UNIT); rt_con_set(some, 1, mkfloat(d)); return some; }\n")
