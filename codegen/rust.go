...
func (Rust) Emit(p Program) (TargetSource, error) {
	...
	// emitted (not just under float IO) so _show's V::Float branch can reach it too --
	...
	// D5 float IO: parseFloat/getFloat/printFloat validation. No regex crate, manual
	// validator (__fmtf, the canonical ECMAScript dressing, is now emitted unconditionally
	// above so _show can reach it too; this block is only the parse-side validator).
