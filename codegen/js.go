...
func (JS) Emit(p Program) (TargetSource, error) {
	...
	em.datas = make(map[string]DataSpec, len(p.Datas))
	for _, d := range p.Datas {
		if d.ElimName != "" && d.ElimName != em.natElim {
			em.datas[d.ElimName] = d
		}
	}
...
type jsEmitter struct {
	...
	// datas maps every emitted eliminator name to its DataSpec so dataDispatch
	// (the natDispatch generalization) can rewrite shallow case-shaped
	// eliminations into constant-time tag dispatch. The builtin-nat eliminator
	// is excluded (its runtime value is a BigInt, not a tagged record; natDispatch
	// owns it).
	datas map[string]DataSpec
}
...
func (em *jsEmitter) expr(t Ir, env []string) string {
	...
	case IApp:
		...
		if out, ok := em.dataDispatch(x, env); ok {
			return out
		}
...
// dataDispatch is natDispatch generalized to every datatype: it recognizes a
// saturated `DElim p… m c… x [extra…]` where each RECURSIVE constructor's arm
// is a literal lambda chain that provably never uses its induction
// hypotheses, and emits a constant-time tag dispatch instead of the recursive
// fold. Without it a shallow `case` on a list (or any recursive datatype)
// pays the full structural fold of the scrutinee's tail at every evaluation —
// the strict-eliminator analogue of the case-on-nat exponential natDispatch
// already fixes. Arms of recursive constructors that are not literal lambdas
// bail to the fold (conservative); unused IHs are fed $unit, exactly the
// natDispatch convention.
func (em *jsEmitter) dataDispatch(app IApp, env []string) (string, bool) {
	if len(em.datas) == 0 {
		return "", false
	}
	var args []Ir
	...
	spec, ok := em.datas[g.Name]
	if !ok {
		return "", false
	}
	np, n := spec.NumParams, len(spec.Ctors)
	need := np + 1 + n + 1
	if len(args) < need {
		return "", false
	}
	arms := args[np+1 : np+1+n]
	scrut := args[np+1+n]
	ihCounts := make([]int, n)
	for i, c := range spec.Ctors {
		own := c.Arity - np
		if len(spec.Rec[i]) < own {
			return "", false
		}
		ihs := 0
		for j := 0; j < own; j++ {
		...
		// Peel the arm's literal lambda chain: own args first, then the IHs
		// (LowerElim's application order). Every IH binder must be unused.
		body := arms[i]
		total := own + ihs
		for k := 0; k < total; k++ {
			lam, isLam := body.(ILam)
		...
		// In the innermost body the IH binders are de Bruijn 0..ihs-1
		// (they are the innermost binders).
		for k := 0; k < ihs; k++ {
			if usesVar(body, k) {
				return "", false
			}
	...
	sv := freshJS("$s", env)
	var b strings.Builder
	fmt.Fprintf(&b, "((%s) => { switch ((%s).tag) {", sv, sv)
	for i, c := range spec.Ctors {
		own := c.Arity - np
		call := em.expr(arms[i], env)
		for j := 0; j < own; j++ {
			call = fmt.Sprintf("%s((%s).args[%d])", call, sv, np+j)
		}
		for k := 0; k < ihCounts[i]; k++ {
			call += "($unit)"
		}
		fmt.Fprintf(&b, " case %d: return %s;", i, call)
	}
	b.WriteString(" } throw new Error('impossible: unmatched constructor tag'); })(")
	b.WriteString(em.expr(scrut, env))
	b.WriteString(")")
	out := b.String()
	for _, extra := range args[need:] {
		out = fmt.Sprintf("%s(%s)", out, em.expr(extra, env))
	}
	return out, true
}
...
func usesVar(t Ir, idx int) bool {
	...
	case IPair:
		return usesVar(x.A, idx) || usesVar(x.B, idx)
	case IFst:
		return usesVar(x.P, idx)
	case ISnd:
		return usesVar(x.P, idx)
	case IField:
		return usesVar(x.Scrut, idx)
	case ICase:
		if usesVar(x.Scrut, idx) {
			return true
		}
		for _, a := range x.Arms {
			if usesVar(a.Body, idx) {
				return true
			}
		}
		return false
	case IBounce:
		return usesVar(x.Call, idx)
	default:
		return false
	}
}
