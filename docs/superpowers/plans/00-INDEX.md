...
> - 6b: the Perceus ownership INSERTION pass - `2026-06-27-perceus-ownership-wasm.md` (PLANNED). DECISIONS (2026-06-27): architecture = EXPLICIT PORTABLE NODES (a CIr->CIr pass inserting CDup/CDrop, opt-in WASM-only; C/LLVM keep mark-sweep; the SAME annotated CIr drives 6e). Scope = RECEIVER-DRIVEN core+structural (user directive: "a receiver for each bit - a story/chapter that covers the case and explains it"): each ownership case ships a runnable receiver listing that balances `$live` (steady-state gate: run main N times, assert flat) + a worked-example section in R-PERCEUS.md. 6 tasks: design doc, CDup/CDrop+emitter+steady-gate, pure fragment (vars/let/app/closures), constructors/case/field (borrowed scrutinee), pairs, corpus gate+6b-2 boundary. HELD for 6b-2 (named receivers): CBounce trampoline ownership (ch39 countdown leak-free) + nat-fold bignum temps (bignum-arith listing leak-free); `perceusBalanceable` predicate makes the boundary executable.
