...
- **Backend count is 9 emitters, 8 full-featured.** JS, Py, Go, Rust, BEAM, JVM, C, LLVM are
  ...
  behavior on "all targets" states which set. 6b-2 (leak residuals) is DONE (v3.337.0), WASM
  partial support landed (v3.341.0), and 6c (refcounted bytes: `Bin` over ARC) is DONE (branch
  `feat/wasm-bin-arc`); the remaining WASM-hardening (6d WebRTC shim) is an explicit dependency
...
> - 6a: ARC heap runtime for WASM (the substrate) - `2026-06-27-arc-wasm-runtime.md` (DONE, v3.336.0). Hidden refcount header, rt_retain/rt_release, recursive child release by kind, size-classed free list@8192, R-ARC.md. WAT-tested via wasmtime (`$live` probe), behavior-preserving. + K_PAIR/K_CLO release tests post-review.
...
>   `420cb7d`..`50bdd09`) - `2026-07-03-wasm-browser-library.md`. The design
>   narrowed from a WebRTC FFI shim to a passive,
...
> - 6f: the two-tab CRDT browser app (DONE, branch `feat/twotab-demo`,
>   `7049c63`..`209ca42`) - `2026-07-03-twotab-demo.md`. `examples/twotab`: a
>   static page (`index.html`/`app.js`) driving a G-Counter compiled to
>   `counter.wasm` via 6d's `EmitLibrary`, BroadcastChannel signaling paired with
>   a real `RTCDataChannel` (`sync.js`), and an optional puppeteer gate
...
>   a `DataCloneError` in the signaling payload (structured-clone cannot carry
>   the shapes `sync.js` was posting) that only surfaces against a real
>   `BroadcastChannel`, fixed by JSON-encoding the wire messages before
>   `postMessage`.
