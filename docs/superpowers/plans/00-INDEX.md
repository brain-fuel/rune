...
- **Backend count is 9 emitters, 8 full-featured.** JS, Py, Go, Rust, BEAM, JVM, C, LLVM are
  ...
  behavior on "all targets" states which set. 6b-2 (leak residuals) is DONE (v3.337.0) and WASM
  partial support landed (v3.341.0); the remaining WASM-hardening (6c refcounted strings/bytes,
  6d WebRTC shim) is an explicit dependency where the demo needs it.
