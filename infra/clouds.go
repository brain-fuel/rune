...
func (AWS) Emit(rs []Resource) (Artifact, error) {
	...
	for _, r := range rs {
		...
		case Stream:
			...
			h.close()
		case CDN:
			h.open("resource \"aws_cloudfront_distribution\" %s", str(v.Name))
			h.attr("enabled", "true")
		...
		case Archive:
			...
			h.close()
		case Serverless:
			h.open("resource \"aws_lambda_function\" %s", str(v.Name))
			h.attr("function_name", str(v.Name))
			h.attr("role", "var.aws_lambda_role_arn")
			h.attr("handler", str("index.handler"))
			h.attr("runtime", str("provided.al2"))
			h.attr("filename", "var.aws_lambda_package")
			h.close()
		case DevOps:
			h.open("resource \"aws_codebuild_project\" %s", str(v.Name))
			h.attr("name", str(v.Name))
			h.attr("service_role", "var.aws_codebuild_role_arn")
			h.open("artifacts")
			h.attr("type", str("NO_ARTIFACTS"))
			h.close()
			h.open("environment")
			h.attr("compute_type", str("BUILD_GENERAL1_SMALL"))
			h.attr("image", str("aws/codebuild/standard:7.0"))
			h.attr("type", str("LINUX_CONTAINER"))
			h.close()
			h.open("source")
			h.attr("type", str("NO_SOURCE"))
			h.attr("buildspec", str("version: 0.2"))
			h.close()
			h.close()
		default:
...
func (Azure) Emit(rs []Resource) (Artifact, error) {
	...
	for _, r := range rs {
		...
		case Stream:
			...
			h.close()
		case CDN:
			h.open("resource \"azurerm_cdn_profile\" %s", str(v.Name))
			h.attr("name", str(v.Name))
			h.attr("resource_group_name", "azurerm_resource_group.wavelet.name")
			h.attr("location", "azurerm_resource_group.wavelet.location")
		...
		case Archive:
			...
			h.close()
		case Serverless:
			h.open("resource \"azurerm_linux_function_app\" %s", str(v.Name))
			h.attr("name", str(v.Name))
			h.attr("resource_group_name", "azurerm_resource_group.wavelet.name")
			h.attr("location", "azurerm_resource_group.wavelet.location")
			h.attr("service_plan_id", "var.azure_service_plan_id")
			h.attr("storage_account_name", "var.azure_storage_account_name")
			h.attr("storage_account_access_key", "var.azure_storage_account_key")
			h.open("site_config")
			h.close()
			h.close()
		case DevOps:
			h.open("resource \"azurerm_container_app_job\" %s", str(v.Name))
			h.attr("name", str(v.Name))
			h.attr("resource_group_name", "azurerm_resource_group.wavelet.name")
			h.attr("location", "azurerm_resource_group.wavelet.location")
			h.attr("container_app_environment_id", "var.azure_container_app_env_id")
			h.attr("replica_timeout_in_seconds", "1800")
			h.open("manual_trigger_config")
			h.close()
			h.open("template")
			h.open("container")
			h.attr("name", str("ci"))
			h.attr("image", str("woodpeckerci/woodpecker-agent:latest"))
			h.attr("cpu", "0.5")
			h.attr("memory", str("1Gi"))
			h.close()
			h.close()
			h.close()
		default:
...
func (GCP) Emit(rs []Resource) (Artifact, error) {
	...
	for _, r := range rs {
		...
		case Stream:
			...
			h.close()
		case CDN:
			h.open("resource \"google_compute_backend_bucket\" %s", str(v.Name))
			h.attr("name", str(v.Name))
		...
		case Archive:
			...
			h.close()
		case Serverless:
			h.open("resource \"google_cloudfunctions2_function\" %s", str(v.Name))
			h.attr("name", str(v.Name))
			h.attr("location", "var.gcp_zone")
			h.open("build_config")
			h.attr("runtime", str("go122"))
			h.attr("entry_point", str("Handler"))
			h.close()
			h.open("service_config")
			h.attr("max_instance_count", "1")
			h.close()
			h.close()
		case DevOps:
			h.open("resource \"google_cloudbuild_trigger\" %s", str(v.Name))
			h.attr("name", str(v.Name))
			h.attr("location", str("global"))
			h.attr("filename", str("cloudbuild.yaml"))
			h.close()
		default:
