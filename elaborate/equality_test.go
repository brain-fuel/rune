...
func TestEqualityStratumPrograms(t *testing.T) {
	cases := []string{
		...
		`p : Eq U U U is refl U end`,
		// Eq lives in Prop; Prop : U.
		`P : U is Prop end
T : Prop is Eq U U U end
p : T is refl U end`,
...
ext : Eq (U -> U) idU idU is refl idU end`,
  ...
  fn {A : U} {x : A} {y : A} (p : Eq A x y) is Eq A y x end
		...
		`transport : (A : U) -> (B : U) -> Eq U A B -> A -> B is
  fn (A : U) (B : U) (p : Eq U A B) (x : A) is cast A B p x end
end
use : U -> U is fn (x : U) is transport U U (refl U) x end end`,
		// UIP at canonical proofs: two refls of the same equation are equal.
		`uipish : (p : Eq U U U) -> Prop is
  fn (p : Eq U U U) is Eq (Eq U U U) (refl U) (refl U) end
end
uipproof : Eq (Eq U U U) (refl U) (refl U) is refl end`,
	}
...
func TestEqualityErrors(t *testing.T) {
	cases := []struct{ src, want string }{
		...
		{`bad : Eq U U (U -> U) is refl end`, "not definitionally equal"},
...
bad : Prop is Eq f f f end`, "not a type"},
		...
		{`bad : Prop is Eq U U end`, "3 arguments"},
...
`
	...
	e, err := surface.ParseExpr(`moved U`)
	if err != nil {
		t.Fatal(err)
	}
	...
	if got := surface.PrettyWith(s.NormalizeExpr(tm), s.RefNames()); got != "U" {
		t.Fatalf("moved U normalized to %q, want U", got)
	}
}
...
func TestProofIrrelevanceAtCasts(t *testing.T) {
	s := session.New()
	src := `
p1 : Eq U U U is refl U end
p2 : Eq U U U is refl U end
eqcasts : (A : U) -> Prop is
  fn (A : U) is Eq U (cast U U p1 A) (cast U U p2 A) end
end
proof : (A : U) -> Eq U (cast U U p1 A) (cast U U p2 A) is
  fn (A : U) is refl end
end
`
	if _, err := s.LoadSource(src); err != nil {
