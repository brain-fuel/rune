...
func (e *Elaborator) CheckCore(c *Ctx, t core.Tm, want core.Val) error {
	...
	case core.Lam(icit, _, body):
		...
		if pi.Icit != icit {
			return fmt.Errorf("%s lambda checked against an %s function type",
				icitName(icit), icitName(pi.Icit))
		}
		v := core.VVar(c.Lvl())
		return e.CheckCore(c.bind(body.Name, pi.Dom), body.Body, pi.Cod(v))
	case core.Let(ty, val, body):
		...
		if ty != nil {
			...
			vty = e.Eval(c, ty)
			if err := e.CheckCore(c, val, vty); err != nil {
				return err
			}
		} else {
			ity, err := e.InferCore(c, val)
			if err != nil {
				return err
			}
			vty = ity
		...
		vval := e.Eval(c, val)
		return e.CheckCore(c.define(body.Name, vty, vval), body.Body, want)
	case _:
