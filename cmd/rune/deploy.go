...
func resourceFor(kind, name string, fifo bool, image string, replicas int) (infra.Resource, error) {
	...
	case "warehouse":
		return infra.Warehouse{Name: name}, nil
	case "inference":
		return infra.Inference{Name: name}, nil
	case "archive":
		return infra.Archive{Name: name}, nil
	case "serverless":
		return infra.Serverless{Name: name}, nil
	case "devops":
		return infra.DevOps{Name: name}, nil
	default:
